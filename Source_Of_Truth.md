...
- Dashboard shell opens one EventSource on `/notifications/stream`. `new_application` invalidates applications lists; `application.scored` patches list/detail cache (invalidate only score-filtered or score-sorted lists). Do not open a second stream per page.
...
| Application | see §11 | links **jobId only**; `roleSnapshot` frozen at apply; `statusHistory[]` append-only `{ status, at }`; `scoring` denormalized on the row; indexes `{ jobId: 1, createdAt: -1 }`, `{ createdAt: 1 }`, `{ "roleSnapshot.roleId": 1, createdAt: -1 }`, `{ "scoring.score": 1, createdAt: -1 }`, and `{ source: 1, campaign: 1 }` |
...
| GET | `/applications` | HR; `q, jobId, roleId, source, campaign, status, sort=createdAt\|score, dir=asc\|desc, scoreMin, scoreMax, scoreLessThan, page, limit≤50` default 15; `roleId` matches `roleSnapshot.roleId` (all jobs for that role); `source` + optional `campaign` (`organic` = null/empty/`organic`); list fields only (includes `score` + `scoringStatus` + `source` + `campaign`); stats via aggregation scoped to the same list filters; `{ applications, stats, pagination }` |
| GET | `/applications/sources` | HR; distinct UTM pairs for the source/campaign filter; `{ sources: [{ source, label, campaigns: [{ key, label }] }] }`; empty/`organic` campaign → key `organic` label Organic |
| POST | `/applications/bulk-reject` | HR; requires `jobId`; optional `q, roleId, status, applicationIds, scoreMin, scoreMax, scoreLessThan, source, campaign`; `dryRun` skips reason; `sendEmail` default true (queued, non-blocking) |
...
| GET | `/notifications/unread-count` `/stream` | SSE `notification` + `application.scored` events |
...
**List UX:** metrics total / opened / avg applicants / closed. Search title + descriptionPlain + ObjectId. Filters: one searchable role dropdown grouped by department (department header filters that dept; a role filters that role) + status (`open` \| `closed` \| `draft`). Columns: title, dept, role, status, type, createdAt, applicants, icon actions. Backend pagination default 15. Filter dropdowns on other pages use `GET /jobs/options`, not the full list.
...
**Application scoring (advisory only):** one denormalized `scoring` object on the Application (`pending` \| `completed` \| `failed`; score 0–10 one decimal, summary, strengths[], gaps[], provider, model, linksAttempted, linksUsed, scoredAt). No separate collection. Never writes `Application.status` or calls `recomputeApplicationStatus`. `completed` is terminal. Retry is HR-only and only when `failed`. Gathering: `extractResumeText` on the stored resume; regex URLs in CV text + `url` custom answers; best-effort link fetch (`extract-link-text`, ~8s, silent drop); JD title/`descriptionPlain`/dept/role; application system fields + experience + education + non-file answers. Each source clipped ~8k chars. One `generateStructured()` call. Malformed JSON / missing key / timeout → `failed` after 4 internal attempts (same posture as email). Boot marks leftover `pending` as `failed` so Retry appears. Logs each step; dumps gathered context before the model call. On `completed` or `failed` (not boot stale-fail), publishes `application.scored` on the existing HR SSE (not a Notification row). Clients patch the list score cell and open detail cache; they do not refetch `GET /applications` unless the active list is filtered or sorted by score.

**HR list:** search name/email; filter job / role / source+campaign / status / score range (below 4, 4–6.9, 7–10). Role dropdown lists every role; `roleId` returns applications for all jobs of that role. Source dropdown is one searchable menu grouped by source (header filters that source; a campaign filters that pair); options from `GET /applications/sources`. Score column uses `StatusPills`; pending = pill-sized shimmer box; failed/empty = em dash. Click **Score** to sort asc/desc (default list remains newest first until Score is clicked). **Source / campaign** column is two lines like Applied At (source bold, campaign faint; missing/`organic` → Organic). **Department / role** is the same (department bold, role faint). Job column header is **Job Label**. Metrics: total, scheduled, rejected, approved (real counts for the current list filters; Total matches the table). Backend pagination default 15. Live: the shared HR SSE invalidates `applications` queries on `new_application` (debounced); the mounted list refetches with current filters. `application.scored` patches `score` + `scoringStatus` on cached rows (and detail `scoring`) instead of refetching. Row click → detail (see HR detail UX above). Every row: view resume, view notes (read-only modal of all interview notes, grouped by interview; fetched on open via `GET /applications/:id/interviews`, not on the list payload). Unlocked row actions: schedule interview, trial (confirm), approve (reason), reject (reason). Approve/reject/bulk-reject modals: heading + close in the header, reason in the body, Send email toggle default on. Trial confirm: heading + close in the header, explanation in the body, no icon. Bulk reject from the filter bar (same list filters, including score and source/campaign). Status pills: submitted sky, under review amber, interview scheduled indigo, interviewed/trial violet, approved green, rejected red.
...
Bell: last 20, mark one/all read, link to `/dashboard/notifications` (search, unread filter, pagination). Unread badge from `/unread-count`. One EventSource per dashboard session (dashboard shell, not the bell). `new_application` also invalidates the applications list (debounced refetch of current filters). `application.scored` is SSE-only (no Notification row, no FCM) and patches the applications list/detail cache.
