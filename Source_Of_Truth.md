...
| GET | `/applications` | HR; `q, jobId, roleId, status, sort=createdAt\|score, dir=asc\|desc, scoreMin, scoreMax, scoreLessThan, page, limit≤50` default 15; `roleId` matches `roleSnapshot.roleId` (all jobs for that role); list fields only (includes `score` + `scoringStatus`); stats via aggregation scoped to the same `q`/`jobId`/`roleId`/`status`/score filters as the list; `{ applications, stats, pagination }` |
